...
Deno.serve(async (request) => {
  ...
  const [organizationResult, entitlementResult, onboardingResult, seatsResult, activityResult, workResult, libraryResult] = await Promise.all([
  ...
  if (organizationResult.error || entitlementResult.error || onboardingResult.error || seatsResult.error || activityResult.error || workResult.error || libraryResult.error) return json({ error: "Unable to load organisation context" }, 500);
